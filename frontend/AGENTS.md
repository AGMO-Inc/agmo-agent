---
type: frontend
---
# AGENTS.md

This file is the working agreement for humans and automation (CI, bots, AI agents).

## Project Overview and Description

- 이 레포는 프론트엔드 애플리케이션/웹 UI 프로젝트다.

## Tools, Technologies, and Frameworks Used

- (이 섹션은 레포의 실제 스택에 맞게 작성한다)
- 예: TypeScript, React/Next.js/Vue, Tailwind/Emotion, Vite, etc.

## How to Build and Run Tests

- (이 섹션은 package manager와 테스트 러너에 맞게 작성한다)
- 예:
  - Install: `pnpm i` / `yarn` / `npm ci`
  - Dev: `pnpm dev`
  - Build: `pnpm build`
  - Test (all): `pnpm test`
  - Test (single): `pnpm test -- <pattern>` (프로젝트의 러너(Jest/Vitest/Playwright)에 맞춰 수정)

## 기본 원칙

- 모든 응답은 한국어로 작성한다.
- 작업 시작/진행/정리 과정에서 TODO-Issue.md 를 단일 진실원천(SSOT)으로 사용한다.
- TODO-Issue.md 에는 "현재 작업 중인 GitHub Issue"에 대한 실행 과제를 기록한다.
- 모든 Git 작업(브랜치 생성/커밋/푸시/PR/리베이스)은 기본적으로 `develop` 또는 `develop` 하위 브랜치에서 수행한다.
- `main`/`master`에서 Git 작업이 필요할 경우에는 사용자에게 명시적으로 허락을 받은 뒤에만 수행한다.

## 레포/프로젝트 정보

- 조직: AGMO-Inc
- 프로젝트명: (여기에 작성)
- 프로젝트 url: (여기에 작성; 관련없으면 N/A)
- 레포: (여기에 작성; `git remote get-url origin`)

## 스킬

- 실행 절차(이슈 시작, 커밋, 푸시/PR 등)는 `.cluade/` 하위 스킬을 참고한다.
- 문서 중복을 피하기 위해 본 파일에는 절차 상세를 중복 기재하지 않는다.
