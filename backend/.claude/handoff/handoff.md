...
- 커밋이 너무 많아. swagger 관련과 .claude 관련으로 두 커밋으로 해줘.

GOAL
----
feature/swagger-#401 브랜치의 2개 커밋(.claude 관련, swagger 관련)을 유지한 채 원격 푸시/PR 및 #404 남은 구현 작업을 이어간다.
...
- 나는 .claude/docs/swagger.md 규칙에 맞춰 User 관련 Request/Response DTO들에 @Schema(클래스/필드) 어노테이션을 보강했다.
...
- develop 대비 커밋 2개:
  - 057a001 chore: 작업 스킬 문서 .claude로 이동
...
- .claude/docs/swagger.md - Swagger 어노테이션/문서 컨벤션(SSOT)
...
- 실행 절차(이슈 시작, 커밋, 푸시/PR)는 `.claude/SKILL.md` 를 참고한다.
- 문서 중복을 피하기 위해 본 파일에는 절차 상세를 중복 기재하지 않는다.
- API 구현 시 (Contoller 클래스 등) .claude/docs/swagger.md 와 .claude/docs/api.md 를 참고한다.
- 코드 작성 시 .claude/docs/code.md 로 컨벤션을 참고한다.
...
- AGENTS.md는 .claude/docs/api.md 를 참조하지만, 현재 워크스페이스에 해당 파일은 보이지 않았다(문서 경로/존재 여부 확인 필요).
