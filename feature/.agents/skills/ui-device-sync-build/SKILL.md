...
description: Build UI artifacts and sync them into a device-served UI directory. Use this when UI and device are a single deployment unit and requests include "ui를 device에 반영", "ui 빌드+동기화", or equivalent phrases.
---
...
```bash
~/.opencode/skills/ui-device-sync-build/scripts/build_and_sync_ui.sh \
...
```bash
# 개발 빌드 + 동기화
~/.opencode/skills/ui-device-sync-build/scripts/build_and_sync_ui.sh --mode dev

# sourcemap 빌드 + 동기화
~/.opencode/skills/ui-device-sync-build/scripts/build_and_sync_ui.sh --mode map
