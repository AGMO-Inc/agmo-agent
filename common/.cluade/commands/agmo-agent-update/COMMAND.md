...
description: 레포의 `.cluade/skills/`, `.cluade/commands/`, `.github/workflows/`를 AGMO-Inc/agmo-agent의 최신 내용과 비교해 차이점을 표로 보고하고, 사용자가 선택한 범위만 안전하게 업데이트하는 커맨드다. `/agmo-agent-update [common|backend|frontend|custom|item-name]` 형태를 지원하며, 업데이트 적용 전 반드시 사용자 선택(전부/없음/부분)을 강제한다.
---

# AGMO Agent Update Command

이 커맨드는 "현재 레포"의 `.cluade/`(skills + commands)와 `.github/workflows/`를 기준으로, 원본 레포 `AGMO-Inc/agmo-agent`와의 차이점을 비교/정리하고 선택된 항목만 업데이트한다.
...
- `/agmo-agent-update common`
  - `common/.cluade/(skills|commands)`와 `common/.github/workflows`를 비교/업데이트 준비한다.
- `/agmo-agent-update backend|frontend|custom`
  - 해당 타입의 `{type}/.cluade/(skills|commands)`만 비교/업데이트 준비한다.
...
### 2) Fetch canonical `.cluade` and workflow templates from AGMO-Inc/agmo-agent
...
- `common skills`: `$root/common/.cluade/skills/`
- `common commands`: `$root/common/.cluade/commands/`
- `type skills`: `$root/<type>/.cluade/skills/`
- `type commands`: `$root/<type>/.cluade/commands/`
...
|---|------|------|--------|--------|------------|----------------|-------|
| 1 | skill | swagger | type=backend | MODIFY | .cluade/skills/swagger | <tmp>/backend/.cluade/skills/swagger | 3 files changed |
| 2 | command | agmo-agent-update | common | ADD | .cluade/commands/agmo-agent-update | <tmp>/common/.cluade/commands/agmo-agent-update | new |
...
1. 백업 생성(선택된 item별로 디렉토리 단위 백업):
   - `.cluade/.backup/agmo-agent-update/<timestamp>/<kind>/<name>/`
    - 여기서 `kind`는 `skills` 또는 `commands` 디렉토리를 의미한다.
2. 업데이트 적용:
   - `ADD`:
     - kind=skill -> canonical dir을 local `.cluade/skills/<name>`로 복사
     - kind=command -> canonical dir을 local `.cluade/commands/<name>`로 복사
    - `MODIFY`:
      - 기존 local dir을 백업 후, canonical dir로 덮어쓰기
   - `workflow`:
     - `MODIFY`만 허용한다.
     - local에 이미 존재하는 `.github/workflows/<name>` 파일만 canonical 템플릿으로 덮어쓴다.
     - local에 없는 workflow는 추가하지 않는다 (`ADD` 금지).
3. 삭제 정책:
...
- 사용자 선택 없이 `.cluade/(skills|commands)/`를 변경하지 않는다.
- 사용자 선택 없이 `.cluade/(skills|commands)/` 또는 `.github/workflows/`를 변경하지 않는다.
- 삭제는 기본 금지(추가 확인 필수).
- `.env`, credentials, secret류 파일을 복사/덮어쓰지 않는다(해당 파일이 감지되면 표에서 경고하고 업데이트 제외).
- 커밋/푸시는 사용자가 명시적으로 요청할 때만 한다.
- workflow 업데이트는 "현재 레포에 이미 존재하는 파일"만 대상으로 하며, 새 workflow 파일은 추가하지 않는다.
