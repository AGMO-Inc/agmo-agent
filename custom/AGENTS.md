...
- 이 레포는 "custom" 타입 프로젝트다. (백엔드/프론트엔드에 속하지 않는 도구/스크립트/인프라/데이터 등)
...
- (이 섹션은 레포의 실제 빌드/테스트 체계에 맞게 작성한다)
- 예:
  - Build: `make build` / `./gradlew build` / `pip -r requirements.txt` 등
  - Test (all): `make test` / `pytest` 등
  - Test (single): 프로젝트에 맞는 단일 테스트 실행 커맨드를 추가한다.
